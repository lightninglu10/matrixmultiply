...
void sgemm( int m, int n, int d, float *A, float *C )
{
	#pragma omp parallel
	{
		__m128 ATvect3j2, ATvect3j3, AT3j3, AT3j2, ATvect2j2, ATvect1j2, ATvect1j3, ATvect2j3, ATj3matrix, ATj2matrix, vect, ATmatrix, ATvect1, ATvect2, ATvect3, ATvect4, ATvect5, ATvect6, ATvect7, Cmatrix, vect2, vect3, vect4, ATvect3j, AT3j, ATvect2j, AT2j, ATvect1j, AT1j, ATjmatrix, ATj;
		#pragma omp for
		for( int j = 0; j < n/4 * 4; j+=4 ) {
			for( int k = 0; k < m/4 * 4; k+=4 ) {
				...
				ATj2matrix =  _mm_load1_ps(A + ((j+2) * (n + 1) + (k) * (n)));
				float ATj2 = A[(j+2)*(n+1)+k*(n)];
				ATj3matrix =  _mm_load1_ps(A + ((j+3) * (n + 1) + (k) * (n)));
				float ATj3 = A[(j+3)*(n+1)+k*(n)];

				ATvect1 =  _mm_load1_ps(A + (j * (n + 1) + (k + 1) * (n)));
				float AT1 = A[j*(n+1)+(k + 1)*(n)];
				ATvect1j =  _mm_load1_ps(A + ((j + 1) * (n + 1) + (k + 1) * (n)));
				float AT1j = A[(j + 1)*(n+1)+(k + 1)*(n)];
				ATvect1j2 =  _mm_load1_ps(A + ((j + 2) * (n + 1) + (k + 1) * (n)));
				float AT1j2 = A[(j + 2)*(n+1)+(k + 1)*(n)];
				ATvect1j3 =  _mm_load1_ps(A + ((j + 3) * (n + 1) + (k + 1) * (n)));
				float AT1j3 = A[(j + 3)*(n+1)+(k + 1)*(n)];

				ATvect2 =  _mm_load1_ps(A + (j * (n + 1) + (k + 2) * (n)));
				float AT2 = A[j*(n+1)+(k + 2)*(n)];
				ATvect2j =  _mm_load1_ps(A + ((j + 1) * (n + 1) + (k + 2) * (n)));
				float AT2j = A[(j + 1)*(n+1)+(k + 2)*(n)];
				ATvect2j2 =  _mm_load1_ps(A + ((j + 2) * (n + 1) + (k + 2) * (n)));
				float AT2j2 = A[(j + 2)*(n+1)+(k + 2)*(n)];
				ATvect2j3 =  _mm_load1_ps(A + ((j + 3) * (n + 1) + (k + 2) * (n)));
				float AT2j3 = A[(j + 3)*(n+1)+(k + 2)*(n)];

				ATvect3 =  _mm_load1_ps(A + (j * (n + 1) + (k + 3) * (n)));
				float AT3 = A[j*(n+1)+(k + 3)*(n)];
				ATvect3j =  _mm_load1_ps(A + ((j + 1) * (n + 1) + (k + 3) * (n)));
				float AT3j = A[(j + 1)*(n+1)+(k + 3)*(n)];
				ATvect1j2 =  _mm_load1_ps(A + ((j + 2) * (n + 1) + (k + 3) * (n)));
				float AT3j2 = A[(j + 2)*(n+1)+(k + 1)*(n)];
				ATvect3j3 =  _mm_load1_ps(A + ((j + 3) * (n + 1) + (k + 3) * (n)));
				float AT3j3 = A[(j + 3)*(n+1)+(k + 3)*(n)];

				for( int i = 0; i < n/8 * 8; i+= 8 ) {
					float *temp = C + i + j * n;
					float *tempj = C + i + (j + 1) * n;
					float *tempj2 = C + i + (j + 2) * n;
					float *tempj3 = C + i + (j + 3) * n;
					float *tmp = A + i + (k)*(n);
				    ...
				    //j + 2
				    Cmatrix = _mm_loadu_ps(tempj2);
				    vect = _mm_mul_ps(_mm_loadu_ps(tmp), ATj2matrix);
				    vect2 = _mm_mul_ps(_mm_loadu_ps(tmp1), ATvect1j2);
				    vect3 = _mm_mul_ps(_mm_loadu_ps(tmp2), ATvect2j2);
				    vect4 = _mm_mul_ps(_mm_loadu_ps(tmp3), ATvect3j2);
				    Cmatrix = _mm_add_ps(Cmatrix, vect);
				    Cmatrix = _mm_add_ps(Cmatrix, vect2);
				    Cmatrix = _mm_add_ps(Cmatrix, vect3);
				    Cmatrix = _mm_add_ps(Cmatrix, vect4);
				    _mm_storeu_ps(tempj2, Cmatrix);
				    // j + 3
				    Cmatrix = _mm_loadu_ps(tempj3);
				    vect = _mm_mul_ps(_mm_loadu_ps(tmp), ATj3matrix);
				    vect2 = _mm_mul_ps(_mm_loadu_ps(tmp1), ATvect1j3);
				    vect3 = _mm_mul_ps(_mm_loadu_ps(tmp2), ATvect2j3);
				    vect4 = _mm_mul_ps(_mm_loadu_ps(tmp3), ATvect3j3);
				    Cmatrix = _mm_add_ps(Cmatrix, vect);
				    Cmatrix = _mm_add_ps(Cmatrix, vect2);
				    Cmatrix = _mm_add_ps(Cmatrix, vect3);
				    Cmatrix = _mm_add_ps(Cmatrix, vect4);
				    _mm_storeu_ps(tempj3, Cmatrix);
				    ...
				    // j + 2
				    Cmatrix = _mm_loadu_ps(tempj2 + 4);
				    vect = _mm_mul_ps(_mm_loadu_ps(tmp + 4), ATj2matrix);
				    vect2 = _mm_mul_ps(_mm_loadu_ps(tmp1 + 4), ATvect1j2);
				    vect3 = _mm_mul_ps(_mm_loadu_ps(tmp2 + 4), ATvect2j2);
				    vect4 = _mm_mul_ps(_mm_loadu_ps(tmp3 + 4), ATvect3j2);
				    Cmatrix = _mm_add_ps(Cmatrix, vect);
				    Cmatrix = _mm_add_ps(Cmatrix, vect2);
				    Cmatrix = _mm_add_ps(Cmatrix, vect3);
				    Cmatrix = _mm_add_ps(Cmatrix, vect4);
				    _mm_storeu_ps(tempj2 + 4, Cmatrix);
				    // j + 3
				    Cmatrix = _mm_loadu_ps(tempj3 + 4);
				    vect = _mm_mul_ps(_mm_loadu_ps(tmp + 4), ATj3matrix);
				    vect2 = _mm_mul_ps(_mm_loadu_ps(tmp1 + 4), ATvect1j3);
				    vect3 = _mm_mul_ps(_mm_loadu_ps(tmp2 + 4), ATvect2j3);
				    vect4 = _mm_mul_ps(_mm_loadu_ps(tmp3 + 4), ATvect3j3);
				    Cmatrix = _mm_add_ps(Cmatrix, vect);
				    Cmatrix = _mm_add_ps(Cmatrix, vect2);
				    Cmatrix = _mm_add_ps(Cmatrix, vect3);
				    Cmatrix = _mm_add_ps(Cmatrix, vect4);
				    _mm_storeu_ps(tempj3 + 4, Cmatrix);
				...
				for (int i = n/8 * 8; i < n; i += 1) {
					...
					C[i+(j + 2)*n] += A[i+k*(n)] * ATj2 + A[i+(k+1)*(n)] * AT1j2 + A[i+(k+2)*(n)] * AT2j2 + A[i+(k+3)*(n)] * AT3j2;
					C[i+(j + 3)*n] += A[i+k*(n)] * ATj3 + A[i+(k+1)*(n)] * AT1j3 + A[i+(k+2)*(n)] * AT2j3 + A[i+(k+3)*(n)] * AT3j3;
				}
			}
			for (int k = m/4 * 4; k < m; k += 1) {
				...
				for( int i = 0; i < n/12 * 12; i+= 12 ) {
				    ...
				    //j + 2
				    Cmatrix = _mm_loadu_ps(tempj2);
				    vect = _mm_mul_ps(_mm_loadu_ps(t2), ATj2matrix);
				    Cmatrix = _mm_add_ps(Cmatrix, vect);
				    _mm_storeu_ps(tempj2, Cmatrix);
				    //j + 3
				    Cmatrix = _mm_loadu_ps(tempj3);
				    vect = _mm_mul_ps(_mm_loadu_ps(t2), ATj3matrix);
				    Cmatrix = _mm_add_ps(Cmatrix, vect);
				    _mm_storeu_ps(tempj3, Cmatrix);
				    ...
				    //j + 2
				    Cmatrix = _mm_loadu_ps(tempj2 + 4);
				    vect = _mm_mul_ps(_mm_loadu_ps(t2 + 4), ATj2matrix);
				    Cmatrix = _mm_add_ps(Cmatrix, vect);
				    _mm_storeu_ps(tempj2 + 4, Cmatrix);
				    //j + 3
				    Cmatrix = _mm_loadu_ps(tempj3);
				    vect = _mm_mul_ps(_mm_loadu_ps(t2 + 4), ATj3matrix);
				    Cmatrix = _mm_add_ps(Cmatrix, vect);
				    _mm_storeu_ps(tempj3 + 4, Cmatrix);
				    ...
				    //j + 2
				    Cmatrix = _mm_loadu_ps(tempj2 + 8);
				    vect = _mm_mul_ps(_mm_loadu_ps(t2 + 8), ATj2matrix);
				    Cmatrix = _mm_add_ps(Cmatrix, vect);
				    _mm_storeu_ps(tempj2 + 8, Cmatrix);
				    //j + 3
				    Cmatrix = _mm_loadu_ps(tempj3 + 8);
				    vect = _mm_mul_ps(_mm_loadu_ps(t2 + 8), ATj3matrix);
				    Cmatrix = _mm_add_ps(Cmatrix, vect);
				    _mm_storeu_ps(tempj3 + 8, Cmatrix);
				}
				for (int i = n/12 * 12; i < n; i += 1) {
					C[i+j*n] += A[i+k*(n)] * AT;
					C[i+(j+1)*n] += A[i+k*(n)] * ATj;
					C[i+(j+2)*n] += A[i+k*(n)] * ATj2;
					C[i+(j+3)*n] += A[i+k*(n)] * ATj3;
				}
			}
		}
	}
